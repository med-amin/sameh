...
add_action( 'customize_register', 'daisy_store_repeater_customize_register' );
function daisy_store_repeater_customize_register(){
/**
 * Repeater control
 */
class Daisy_Store_Control_Repeater extends WP_Customize_Control {
	...
	/**
	 * The daisy_store_config we're using for this control
	 *
	 * @access public
	 * @var string
	 */
	public $daisy_store_config = 'global';
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->row_label = array(
			'type' => 'text',
			'value' => esc_attr__( 'row', 'daisy-store' ),
			'field' => false,
		);
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'daisy-store' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_attr__( 'Select a Page', 'daisy-store' ),
								'option_none_value' => '0',
								'selected'          => '',
							)
						);
	...
	public function enqueue() {
				...
				if ( isset( $field['type'] ) ) {
					...
					switch ( $field['type'] ) {
						...
						case 'dropdown-pages':
							...
							break;
						case 'iconpicker':
							wp_enqueue_script( 'daisy-store-fontawesome-iconpicker', get_template_directory_uri() . '/inc/customizer-library/custom-controls/iconpicker-control/assets/js/fontawesome-iconpicker.min.js', array( 'jquery' ), '1.0.0', true );
							wp_enqueue_script( 'daisy-store-iconpicker-control', get_template_directory_uri() . '/inc/customizer-library/custom-controls/iconpicker-control/assets/js/iconpicker-control.js', array( 'jquery' ), '1.0.0', true );
							
							wp_localize_script( 'daisy-store-iconpicker-control', 'daisy_store_iconpicker', array( 'icons' => daisy_store_fontawsome_icons(),)  );
			
							wp_enqueue_style( 'daisy-store-fontawesome-iconpicker', get_template_directory_uri() . '/inc/customizer-library/custom-controls/iconpicker-control/assets/css/fontawesome-iconpicker.min.css' );
							wp_enqueue_style( 'daisy-store-fontawesome', get_template_directory_uri() . '/inc/customizer-library/custom-controls/iconpicker-control/assets/css/font-awesome.min.css' );
						break;
		...
		wp_enqueue_script( 'daisy-store-repeater', CUSTOMIZER_LIB_URI. '/custom-controls/repeater/repeater.js', array( 'jquery', 'customize-base', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
		wp_enqueue_style( 'daisy-store-repeater-css', CUSTOMIZER_LIB_URI . '/custom-controls/repeater/repeater.css', null );
	}
	...
	protected function render_content() {
		?>
		<div class="dstore-controls-loading-spinner"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div>
		<label>
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit" data-limit="<?php echo absint( $this->choices['limit'] ); ?>"><?php printf( esc_attr__( 'Limit: %s rows', 'daisy-store' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-header">
					...
					<span class="repeater-row-remove" style="cursor:pointer;"><?php esc_attr_e( 'Remove', 'daisy-store' ); ?></span>
				...
				<div class="repeater-row-content">
						...
						<div class="repeater-field repeater-field-{{{ field.type }}}">
							...
							<# } else if ( 'color' === field.type ) { #>
								...
								<label>
									<# if ( field.label ) { #><span class="customize-control-title">{{{ field.label }}}</span><# } #>
									<# if ( field.description ) { #><span class="description customize-control-description">{{{ field.description }}}</span><# } #>
									<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'daisy-store' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
							...
							<# } else if ( 'textarea' === field.type ) { #>
								...
								<# } else if ( 'editor' === field.type ) { #>
									 <div class="customize-control-dstore-editor">
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="dstore-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'daisy-store' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_attr_e( 'No Image Selected', 'daisy-store' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'daisy-store' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'daisy-store' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'daisy-store' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change Image', 'daisy-store' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add Image', 'daisy-store' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="dstore-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'daisy-store' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_attr_e( 'No File Selected', 'daisy-store' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'daisy-store' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'daisy-store' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change File', 'daisy-store' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add File', 'daisy-store' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'daisy-store' ); ?></button>
				</div>
			</li>
		</script>
		<?php
