...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	 ...
	 * WC_List_Grid class
	 **/
	if ( ! class_exists( 'Daisy_Store_List_Grid' ) ) {

		class Daisy_Store_List_Grid {
			...
			function setup_gridlist() {
				...
				if ( is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() ) {
					...
					add_action( 'daisy_store_after_loop_title', 'woocommerce_template_single_excerpt', 5);
		...
		$Daisy_Store_List_Grid = new Daisy_Store_List_Grid();
	}
}
